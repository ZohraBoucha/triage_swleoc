...
def categorize_pdf(filename):
    if re.search(r'R\d+', filename, re.IGNORECASE):
        return 'R'
    elif re.search(r'C\d+', filename, re.IGNORECASE):
        return 'C'
    else:
        return 'Unknown'
...
def process_hospital_folders(root_dir):
    ...
    for hospital_folder in folders:
        ...
        for filename in pdf_files:
            ...
            if category == 'C':
                match = re.search(r'C(\d+)', filename, re.IGNORECASE)
                if match:
                    number = match.group(1)
                    outcomes[number] = extract_outcome(filename)
                    print(f"Extracted outcome from 'C' file: {filename}")
            elif category == 'R':
